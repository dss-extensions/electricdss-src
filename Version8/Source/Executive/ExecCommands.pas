...
{Do Nothing - comment};
   ...
   // Process the rest of the commands
       ...
       {28: DoAboutBox; }
            ...
            31:
                ActiveCircuit[ActiveActor].InvalidateAllPCElements;  // FORce rebuilding of Y
            32:
                CmdResult := DoGetCmd;
            33:
...
{$ELSE}
            112:
            begin
                DSSInfoMessageDlg('Winsock TCP/IP connection is not supported in FPC version, it will be migrated to Indy (soon...)');
                CmdResult := 0;
            end;
            113:
            begin
                DSSInfoMessageDlg('Winsock TCP/IP disconnection is not supported in FPC version, it will be migrated to Indy (soon...)');
                CmdResult := 0;
            end;
{$ENDIF}
            115:
       ...
       // Ignore excess parameters
        ...
        On E: Exception do
            DoErrorMsg(('ProcessCommand' + CRLF + 'Exception Raised While Processing DSS Command:' + CRLF + parser[ActiveActor].CmdString),
                E.Message,
