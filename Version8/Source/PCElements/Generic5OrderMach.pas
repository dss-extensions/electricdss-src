        ...
        begin
         ...
         // use PropertyIdxMap to map to the correct Case index
            ...
            //9: MachineData.D       := Parser.DblValue;
                    10:
                        P_ref1 := 1000 * Parser[ActorID].DblValue;  //for phase ctrl unit kW to W
                    11:
                        P_ref2 := 1000 * Parser[ActorID].DblValue;  //for phase ctrl
                    12:
                        P_ref3 := 1000 * Parser[ActorID].DblValue;  //for phase ctrl
                    13:
                        V_ref1 := 1000 * Parser[ActorID].DblValue;  //for phase ctrl unit kV to V
                    14:
                        V_ref2 := 1000 * Parser[ActorID].DblValue;  //for phase ctrl
                    15:
                        V_ref3 := 1000 * Parser[ActorID].DblValue;  //for phase ctrl
                    16:
              ...
              {}
                    22:
                        Set_P_Ref(Parser[ActorID].DblValue, ActorID);//to norm value W from kW(in script)//for avg ctrl    1000*PrefKw/3;
                    23:
                        Set_Q_Ref(Parser[ActorID].DblValue);//to VA from kVA(in script)         //for avg ctrl    1000*QrefKVAr/3;
                    24:
                        Cluster_Num := Parser[ActorID].Intvalue;
                    25:
                        Set_V_Ref(Parser[ActorID].Dblvalue);//kV  to V                          //for avg ctrl    1000*VrefkV;
                    26:
                        ctrl_mode := Parser[ActorID].Intvalue;
                ///////////////////////////////////////////
    ///
    ///////////////////////////////////////////
                    27:
                    begin
                        QV_flag := Parser[ActorID].Intvalue;
                    end;  // QV_flag_0 :=QV_flag
                    28:
                        kcd := Parser[ActorID].dblvalue;
                    29:
                        kcq := Parser[ActorID].dblvalue;
                    30:
                        kqi := Parser[ActorID].dblvalue;
                    31:
                        Q_ref1 := 1000 * Parser[ActorID].DblValue; //for phase ctrl unit kVar to Var
                    32:
                        Q_ref2 := 1000 * Parser[ActorID].DblValue; //for phase ctrl
                    33:
                        Q_ref3 := 1000 * Parser[ActorID].DblValue; //for phase ctrl
                    34:
                    begin
                    ...
                    36:
                        PQpriority := Parser[ActorID].intValue;  //
                    37:
                        Pmpp := 1000 * Parser[ActorID].DblValue;  //for pmpp kW
                    38:
                        Pfctr1 := Parser[ActorID].DblValue;  //for pmpp
                    39:
                        Pfctr2 := Parser[ActorID].DblValue;  //for pmpp
                    40:
                        Pfctr3 := Parser[ActorID].DblValue; //for pmpp
                    41:
                        Pfctr4 := Parser[ActorID].DblValue; //for pmpp
                    42:
                        Pfctr5 := Parser[ActorID].DblValue; //for  pmpp
                    43:
                        Pfctr6 := Parser[ActorID].DblValue; //for pmpp
                    44:
                        Pbias := 1000 * Parser[ActorID].DblValue; //for pmpp
                    45:
                        CC_switch := InterpretYesNo(parser[ActorID].StrValue); //  yes, true, y ,t; or no, false, n, f
                    46:
                        kcq_drp2 := parser[ActorID].DblValue; //cluster num
                    47:
                        Volt_Trhd := parser[ActorID].DblValue;
                    48:
                        droop := Parser[ActorID].intValue;
                else
...
var
    ...
    i, j: Integer;
begin
    if ActiveCircuit[ActorID].Solution.Dynavars.IterationFlag = 0 then
    begin
