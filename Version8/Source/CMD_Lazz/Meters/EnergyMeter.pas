...
uses
    ...
    LineUnits,
    Classes,
    ReduceAlgs, {*LCLIntf, LCLType,*} Math; // TEMc
...
begin
    ...
    begin  {Write Totals Demand interval file}
        with ActiveCircuit.Solution do
            Write(FDI_Totals, Format('%-.6g ', [DynaVars.dblHour]));
        for i := 1 to NumEMRegisters do
            Write(FDI_Totals, Format(', %-.6g', [DI_RegisterTotals[i]]));
        Writeln(FDI_Totals);
        ClearDI_Totals;
        ...
        // attach them as generic objects to cktTree node.
            ...
            //  IF pPCelem.Enabled Then Begin   only enabled elements in the search list
                ...
                // Is this a load or a generator or a Capacitor or reactor??
                            ...
                            if (PCElementType = LOAD_ELEMENT) or (PCElementType = GEN_ELEMENT) or (PCElementType = CAP_ELEMENT) or (PCElementType = REACTOR_ELEMENT) then
                            begin
...
{Run down the zone and write each element into a file}
    ...
    {Open some files:}
     ...
     {If any records were written to the file, record their relative names}
        if NBranches > 0 then
            SavedFileList.Add(dirname + '\Branches.dss')
        else
            DeleteFile('Branches.dss');
        if NXfmrs > 0 then
            SavedFileList.Add(dirname + '\Transformers.dss')
        else
            DeleteFile('Transformers.dss');
        if NShunts > 0 then
            SavedFileList.Add(dirname + '\Shunts.dss')
        else
            DeleteFile('Shunts.dss');
        if NLoads > 0 then
            SavedFileList.Add(dirname + '\Loads.dss')
        else
            DeleteFile('Loads.dss');
        if NGens > 0 then
            SavedFileList.Add(dirname + '\Generators.dss')
        else
            DeleteFile('Generators.dss');
        if NCaps > 0 then
            SavedFileList.Add(dirname + '\Capacitors.dss')
        else
...
begin
    ...
    for i := 1 to NumEMregisters do
        Write(energyMeterClass.FMeterTotals, Format(', %-g', [Registers[i]]));
         ...
         {Phase Voltage Report, if requested}
            ...
            begin
                ...
                begin
                    ...
                    begin
                        for j := 1 to 3 do
                            Write(VPhase_File, Format(', %.3gkV_Phs_%d_Max', [vbase, j]));
                        for j := 1 to 3 do
                            Write(VPhase_File, Format(', %.3gkV_Phs_%d_Min', [vbase, j]));
                        for j := 1 to 3 do
                            Write(VPhase_File, Format(', %.3gkV_Phs_%d_Avg', [vbase, j]));
                    end;
                end;
...
begin
    ...
    begin
        with ActiveCircuit.Solution do
            Write(DI_File, Format('%-.6g', [DynaVars.dblHour]));
        for i := 1 to NumEMRegisters do
            Write(DI_File, Format(', %-.6g', [Derivatives[i]]));
        Writeln(DI_File);
    end;
    ...
    begin
        with ActiveCircuit.Solution do
            Write(VPhase_File, Format('%-.6g', [DynaVars.dblHour]));
        for i := 1 to MaxVBaseCount do
            if VBaseList^[i] > 0.0 then
            begin
                for j := 1 to 3 do
                    Write(VPhase_File, Format(', %-.6g', [0.001 * VPhaseMax^[jiIndex(j, i)]]));
                for j := 1 to 3 do
                    Write(VPhase_File, Format(', %-.6g', [0.001 * VPhaseMin^[jiIndex(j, i)]]));
                for j := 1 to 3 do
                    Write(VPhase_File, Format(', %-.6g', [0.001 * MyCount_Avg(VPhaseAccum^[jiIndex(j, i)], VPhaseAccumCount^[jiIndex(j, i)])]));
            end;
        Writeln(VPhase_File);
    end;
    ...
    begin
        ...
        begin   // Ignore shunts
            ...
            begin
                ...
                begin
                    with ActiveCircuit.Solution do
                        Write(FOverLoadFile, Format('%-.6g,', [DynaVars.dblHour]));
                    Write(FOverLoadFile, Format(' %s, %-.4g, %-.4g,', [FullName(PDelem), PDElem.NormAmps, pdelem.EmergAmps]));
                    if PDElem.Normamps > 0.0 then
                        Write(FOverLoadFile, Format(' %-.7g,', [Cmax / PDElem.Normamps * 100.0]))
                    else
                        Write(FOverLoadFile, ' 0.0,');
                    if PDElem.Emergamps > 0.0 then
                        Write(FOverLoadFile, Format(' %-.7g,', [Cmax / PDElem.Emergamps * 100.0]))
                    else
                        Write(FOverLoadFile, ' 0.0,');
...
begin
    try
        ...
        if Assigned(mtr) then
            for i := 1 to NumEMRegisters do
                Write(FDI_Totals, ', "', mtr.RegisterNames[i], '"');
        Writeln(FDI_Totals);
    except
        On E: Exception do
            DoSimpleMsg('Error creating: "' + DI_Dir + '\DI_Totals.CSV": ' + E.Message, 539)
    end;
end;
...
begin
    ...
    if Assigned(mtr) then
        for i := 1 to NumEMRegisters do
            Write(FMeterTotals, ', "', mtr.RegisterNames[i], '"');
...
begin
    with ActiveCircuit.Solution do
        Write(SystemDIFile, Format('%-.6g', [DynaVars.dblHour]));
...
begin
  {Sum up all registers of all meters and write to Totals.CSV}
    ...
    try     // Write the file
        ...
        if assigned(mtr) then
            for i := 1 to NumEMRegisters do
                Write(F, ', "', mtr.RegisterNames[i], '"');
        Writeln(F);
        Write(F, ActiveCircuit.Solution.Year: 0);
        for i := 1 to NumEMRegisters do
            Write(F, Format(', %-g ', [RegSum[i]]));
        Writeln(F);
        CloseFile(F);
    except
        On E: Exception do
            DosimpleMsg('Error opening demand interval file Totals.CSV.' + CRLF + E.Message, 543);
    end;
...
begin
    ...
    begin
        ...
        with Solution do
            Write(FVoltageFile, Format('%-.6g,', [DynaVars.dblHour]));
...
begin
    try
        ...
        On E: Exception do
            DosimpleMsg('Error opening demand interval file "' + EnergyMeterClass.DI_Dir + '\DI_Overloads.CSV"  for writing.' + CRLF + E.Message, 541);
    end;
...
begin
    try
        ...
        On E: Exception do
            DosimpleMsg('Error opening demand interval file "' + EnergyMeterClass.DI_Dir + '\DI_VoltExceptions.CSV"  for writing.' + CRLF + E.Message, 541);
    end;
