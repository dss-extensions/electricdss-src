            ...
            if BusIdx > 0 then
                with  ActiveCircuit.Buses^[BusIdx] do
                    if CoordDefined then
                        SysUtils.StrlCopy(Name1, pAnsiChar(Ansistring(ActiveCircuit.BusList.Get(Busidx))), Len1);
      {Second bus}
            BusIdx := CktElement.Terminals^[2].busref;
            if BusIdx > 0 then
                with  ActiveCircuit.Buses^[BusIdx] do
                    if CoordDefined then
                        SysUtils.StrlCopy(Name2, pAnsiChar(Ansistring(ActiveCircuit.BusList.Get(Busidx))), Len2);
        end; {If CktElement}
