        ...
        begin
            ...
            case ParamPointer of
                ...
                begin
                    ...
                    if CompareTextShortest(Parser.StrValue, 'voltwatt') = 0 then
                    begin
                        ControlMode := 'VOLTWATT';
                        CombiControlMode := '';
                    end
                    else
                    if CompareTextShortest(Parser.StrValue, 'dynamicreaccurr') = 0 then
                    begin
                        ControlMode := 'DYNAMICREACCURR';
                        CombiControlMode := '';
                    end
                    else
                    if CompareTextShortest(Parser.StrValue, 'fixedpf') = 0 then
                    begin
                        ControlMode := 'FIXEDPF';
                        CombiControlMode := '';
                    end;
                ...
                begin
                    ...
                    if CompareTextShortest(Parser.StrValue, 'vv_drc') = 0 then
                    begin
                        ControlMode := '';
                ...
                6:
                    if CompareTextShortest(Parser.StrValue, 'rated') = 0 then
                        FVoltage_CurveX_ref := 0
                    else
                        FVoltage_CurveX_ref := 1;
                7:
                    FRollAvgWindowLength := InterpretAvgVWindowLen(Param);
                8:
                begin
                ...
                11:
                    FArGraLowV := Parser.DblValue;
                12:
                    FArGraHiV := Parser.DblValue;
                13:
                    FDRCRollAvgWindowLength := InterpretDRCAvgVWindowLen(Param);
                14:
                    FdeltaQ_factor := Parser.DblValue;
                15:
                    FVoltageChangeTolerance := Parser.DblValue;
                16:
                    FVarChangeTolerance := Parser.DblValue;
                17:
                begin
                    if CompareTextShortest(Parser.StrValue, 'pmpppu') = 0 then
                        FVoltwattYAxis := 1
                    else
                    if CompareTextShortest(Parser.StrValue, 'pavailablepu') = 0 then
                        FVoltwattYAxis := 0
                    else
                    if CompareTextShortest(Parser.StrValue, 'pctpmpppu') = 0 then
                        FVoltwattYAxis := 2
                end;

                18:
                begin
                    if CompareTextShortest(Parser.StrValue, 'inactive') = 0 then
                        RateofChangeMode := INACTIVE
                    else
                    if CompareTextShortest(Parser.StrValue, 'lpf') = 0 then
                        RateofChangeMode := LPF
                    else
                    if CompareTextShortest(Parser.StrValue, 'risefall') = 0 then
                ...
                begin
                    if Parser.DblValue > 0 then
                        FRiseFallLimit := Parser.DblValue
                    else
                        RateofChangeMode := INACTIVE;
                end;
                21:
                    FdeltaP_factor := Parser.DblValue;
                22:
                    ShowEventLog := InterpretYesNo(param);
                23:
                begin
                    if CompareTextShortest(Parser.StrValue, 'varaval_watts') = 0 then
                        FVV_ReacPower_ref := 'VARAVAL_WATTS'
                    else
                    if CompareTextShortest(Parser.StrValue, 'varmax_vars') = 0 then
                        FVV_ReacPower_ref := 'VARMAX_VARS'
                    else
                    if CompareTextShortest(Parser.StrValue, 'varmax_watts') = 0 then
                        FVV_ReacPower_ref := 'VARMAX_WATTS'
                end;
            else
    ...
    if OtherInvControl <> NIL then
        ...
        begin
            ...
            for j := 1 to ParentClass.NumProperties do
                PropertyValue[j] := OtherInvControl.PropertyValue[j];

        end
    else
        DoSimpleMsg('Error in InvControl MakeLike: "' + InvControlName + '" Not Found.', 370);
              ...
              // if the desired kW and desired kvar exceed the kva rating of the PVSystem's inverter then...
                ...
                begin
                  ...
                  //...else, vars have precedence, reduce the active power to not exceed the kva rating
                        ...
                        PTemp := 0.99 * sign(PTemp) * SQRT(Sqr(PVSys.kVARating) - Sqr(QNew[k]));
                    FHitkvaLimit[k] := TRUE;
                end;
...
begin
    ...
    case Index of
        ...
        8:
            Result := Format('%s', [Fvoltwatt_curvename]);
        9:
        ...
        18:
        begin
            if RateofChangeMode = INACTIVE then
                Result := 'INACTIVE'
            else
...
begin
    ...
    if (FDRCRollAvgWindow[j].Get_AvgVal / (basekV * 1000.0)) = 0.0 then
        deltaVDynReac[j] := 0
    else
        deltaVDynReac[j] := FPresentVpu[j] - FDRCRollAvgWindow[j].Get_AvgVal / (basekV * 1000.0);
    ...
    if FVV_ReacPower_ref = 'VARAVAL_WATTS' then
        TempQ := QDRCDesiredpu[j] * PVSys.kVARating
    else
        TempQ := QDRCDesiredpu[j] * PVSys.kvarLimit;
