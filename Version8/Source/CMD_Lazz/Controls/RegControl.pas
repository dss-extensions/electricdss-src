...
begin
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    begin
                        if (DebugTrace) then
                            RegWriteDebugRecord(Format('Pushing Reverse Action, FwdPower=%.8g', [FwdPower]));
                        ReversePending := TRUE;
                        with ActiveCircuit do
            ...
            begin   // If reversed look to see if power is back in forward direction
                ...
                begin
                    ...
                    begin
                        if (DebugTrace) then
                            RegWriteDebugRecord(Format('Pushing Reverse Action to switch back, FwdPower=%.8g', [FwdPower]));
                        ReversePending := TRUE;
                        with ActiveCircuit do
                            ControlQueue.Push(Solution.DynaVars.intHour, Solution.DynaVars.t + RevDelay, ACTION_REVERSE, 0, Self);
                    end
    ...
    begin
        ...
        if (Abs(Vreg - Vactual) > Bandwidth / 2.0) then
