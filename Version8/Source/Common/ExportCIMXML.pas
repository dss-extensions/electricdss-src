...
begin
    ...
    if pElem.NPhases = 2 then
        if ActiveCircuit[ActiveActor].Buses^[pElem.Terminals^[bus].BusRef].kVBase < 0.25 then
            bSec := TRUE;
    if pElem.NPhases = 1 then
        if ActiveCircuit[ActiveActor].Buses^[pElem.Terminals^[bus].BusRef].kVBase < 0.13 then
            bSec := TRUE;
...
var
    ...
    pPhase: TNamedObject;
begin
    if pLine.NPhases = 3 then
        if pLine.NumConductorsAvailable = 0 then
            exit;
...
begin
    ...
    {$ENDIF}
        ...
        ActiveCircuit[ActiveActor].GUID := FdrGuid;
