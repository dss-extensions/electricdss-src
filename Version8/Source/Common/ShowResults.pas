...
begin
    ...
    begin
        ...
        if Node1 > 0 then
            Bus1 := ActiveCircuit[ActiveActor].MapNodeToBus^[Node1].BusRef
        else
            Bus1 := 0;
        if Node2 > 0 then
            Bus2 := ActiveCircuit[ActiveActor].MapNodeToBus^[Node2].BusRef
        else
            Bus2 := 0;
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                if { (p_Elem.nphases=1) and } ActiveCircuit[ActiveActor].PositiveSequence then
                                    S := CmulReal(S, 3.0);
                                if Opt = 1 then
                                    S := CmulReal(S, 0.001);
                                Caccum(Saccum, S);
...
begin
    ...
    try
        ...
        begin
            ...
            Volts := ActiveCircuit[ActiveActor].Solution.NodeV^[nref];
            S := Cmul(Volts, conjg(c_Buffer^[k]));
            if { (CktElem.nphases=1) and } ActiveCircuit[ActiveActor].PositiveSequence then
                S := CmulReal(S, 3.0);
            if Opt = 1 then
                S := CmulReal(S, 0.001);
            Caccum(Saccum, S);
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        Writeln(F, UpperCase(ActiveDssObject[ActiveActor].Name));   // non cktelements
                end;
            end;
        end
        else
...
begin
    ...
    try
        ...
        begin
            ...
            DoSimpleMsg('Meter Name Not Specified.' + CRLF + parser[ActiveActor].CmdString, 221);
...
begin
    ...
    if ActiveCircuit[ActiveActor].BusNameRedefined then
...
begin
    ...
    try
        ...
        {Check Loads}
        if ActiveCircuit[ActiveActor].Loads.ListSize > 0 then
        begin
            Writeln(F);
        ...
        if ActiveCircuit[ActiveActor].Generators.ListSize > 0 then
        begin
            Writeln(F);
