    ...
    begin
        ...
        begin
            ...
            case ParamPointer of
                ...
                5:
                    StdDev := Parser[ActorID].Dblvalue * 0.01;
                6:
                    DoGmatrix(ActorID);
                7:
                    ON_Time := Parser[ActorID].Dblvalue;
                8:
                    IsTemporary := InterpretYesNo(Param);
                9:
                    MinAmps := Parser[ActorID].DblValue;
            else
           // Inherited
...
begin
  ...
  // make sure randommult is 1.0 if not solution mode MonteFault
    ...
    if ActiveCircuit[ActorID].Solution.Mode <> MONTEFAULT then
        RandomMult := 1.0;
